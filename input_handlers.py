...
class AskUserEventHandler(EventHandler):
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[Action]:
        """By default, mouse clicks exit."""
        return self.on_exit()

    def on_exit(self) -> Optional[Action]:
        """Called when user is trying to exit or cancel an action.

        By default, return to main event handler.
        """
        self.engine.event_handler = MainGameEventHandler(self.engine)
        return None
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    What happens next depends on the subclass.
    """
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        # Put on the left if player is on the right,
        # put on the right if player is on the left.
...
class SelectIndexHandler(AskUserEventHandler):
    """ Handles asking users to select indices on the map. """
    def __init__(self, engine: Engine):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action]:
        ...
        key = event.sym
        if key in MOVE_KEYS:
            modifier = 1  # use modifier keys to speed up key movement.
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[Action]:
        ...
        return super().evmousebuttondown(event)
...
class SingleRangedAttackHandler(SelectIndexHandler):
    """ Handles targeting a single enemy; only affects the targeted enemy. """
...
class AreaRangedAttackHandler(SelectIndexHandler):
    """ Targets all actors within a radius. """
